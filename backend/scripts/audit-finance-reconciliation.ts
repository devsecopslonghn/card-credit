...
try {
  ...
  const [statements, finance] = await Promise.all([
  ...
  console.log(JSON.stringify({ workspaceId, sourceOfTruth: { balances: "accounts + financialtransactions", statementStatus: "cardstatements" }, counts: { statements: statements.length, paidStatements: paidIds.size, financialTransactions: finance.length, paidForOther: finance.filter((item) => item.ownership === "PAID_FOR_OTHER").length, reimbursements: finance.filter((item) => item.transactionType === "REIMBURSEMENT").length }, statementSync: { paidWithAmount: statements.filter((item) => item.paymentStatus === "PAID" && Number(item.paidAmount ?? 0) > 0).length, syncedPayments: syncedIds.size, missingPayments: [...paidIds].filter((id) => !syncedIds.has(id)) }, receivable: { gross: grossReceivable, recordedReimbursements: reimbursements, net: Math.max(0, grossReceivable - reimbursements) }}));
