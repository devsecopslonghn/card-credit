...
test("projection creates two deterministic provider-neutral events without raw identity", () => {
  const events = projectStatementCalendar(input);
  assert.equal(events.length, 2);
  assert.deepEqual(events.map((event) => event.type), ["statement", "payment-due"]);
  assert.deepEqual(events.map((event) => event.date), ["2028-02-29", "2028-12-31"]);
  assert.match(events[0]!.title, /^Chốt sao kê/);
  assert.match(events[1]!.title, /^Hạn thanh toán/);
...
test("serializer emits two all-day events with CRLF, folding, escaping and safe stable UIDs", () => {
  ...
  assert.match(calendar, /SUMMARY:Chốt sao kê – Thẻ Việt\\, Platinum\\; Plus/);
...
test("serializer handles non-leap month boundaries and all text escape classes", () => {
  const events = projectStatementCalendar({ ...input, statementDate: "2027-02-28", displayName: "Thẻ \\ A, B; C\nD" });
...
test("email composer creates safe Vietnamese multipart content and calendar filename", () => {
  ...
  assert.match(email.subject, /^Lịch sao kê và thanh toán/);
  ...
  assert.match(email.attachment.filename, /^lich-sao-ke-[a-z0-9-]+-2028-02\.ics$/);
