...
export const registerMcpTools = (server: McpServer, ctx: ContextProvider) => {
  ...
  server.registerTool("get_statement_summary", { description: "Read a workspace-scoped credit-card statement summary from Financial Domain.", inputSchema: { statementId: z.string().min(1) } }, async ({ statementId }) => json(await FinancialReportService.statementSummary(await invocationContext(), statementId)));
  server.registerTool("list_transactions", { description: "List all workspace-scoped financial transactions from Financial Domain, including debit, cash and credit.", inputSchema: { date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), accountId: z.string().min(1).optional(), categoryId: z.string().min(1).optional() } }, async (filters) => json(await FinancialTransactionService.list(await invocationContext(), { from: filters.date, to: filters.date, accountId: filters.accountId, categoryId: filters.categoryId })));
  server.registerTool("compare_cards", { description: "Compare active cards in the fixed workspace.", inputSchema: {} }, async () => json(await CardService.compare(await invocationContext())));
  server.registerTool("list_upcoming_statements", { description: "List unpaid statements from Financial Domain ordered by payment due date.", inputSchema: { limit: z.number().int().min(1).max(50).default(20) } }, async ({ limit }) => json(await FinancialReportService.upcomingStatements(await invocationContext(), limit)));
  server.registerTool("get_personal_finance_summary", { description: "Read separated personal spending, debit/cash flow and credit debt.", inputSchema: { from: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), to: z.string().regex(/^\d{4}-\d{2}-\d{2}$/) } }, async ({ from, to }) => json(await FinancialReportService.summary(await invocationContext(), { from, to })));
