...
export const registerMcpTools = (server: McpServer, ctx: ContextProvider) => {
  ...
  server.registerTool("get_statement_summary", mcpToolMetadata("get_statement_summary"), async ({ statementId }: { statementId: string }) => json(await FinancialReportService.statementSummary(await invocationContext(), statementId)));
  ...
  server.registerTool("list_upcoming_statements", mcpToolMetadata("list_upcoming_statements"), async ({ limit }: { limit: number }) => json(await FinancialReportService.upcomingStatements(await invocationContext(), limit)));
