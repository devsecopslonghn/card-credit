...
export const registerMasterdataRoutes = (app: FastifyInstance, repository: MasterdataRepository, secret: string, users: Pick<AuthRepository, "findUserById">) => {
  ...
  app.get("/api/banks", async (request) => { await context(request); return repository.list("banks", "shortname"); });
  ...
  app.get("/api/cardtypes", async (request) => { await context(request); return repository.list("cardtypes", "name"); });
