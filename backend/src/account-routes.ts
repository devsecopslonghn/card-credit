...
export const registerAccountRoutes = (app: FastifyInstance, secret: string) => {
  app.get("/api/accounts", async (request) => {
    return { data: await AccountService.list(browserServiceContext(request, secret)) };
  });
  ...
  app.post("/api/accounts", async (request, reply) => {
    ...
    return reply.status(201).send({
      data: await AccountService.create(browserServiceContext(request, secret), parsed.data as CreateAccountInput),
    });
  });
};
