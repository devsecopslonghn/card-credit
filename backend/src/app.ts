...
export const buildApp = (database: Pick<DatabaseLifecycle, "isReady">, logLevel = "info", catalog: CatalogRepository = new InMemoryCatalogRepository(), authSecret = "test-secret-at-least-thirty-two-characters", writeAudit: CatalogAuditWriter = async () => {}) => {
  ...
  app.get("/api/admin/card-catalog/products", async (request) => { const session = admin(request); return { data: (await catalog.listAllProducts()).map(withLegacyAliases), audit: auditFor(session) }; });
  ...
  app.patch<{ Params: { presetId: string }; Body: Record<string, unknown> }>("/api/admin/card-catalog/products/:presetId", async (request) => { const session = admin(request); const existing = (await catalog.listAllProducts()).find((p) => p.presetId === request.params.presetId); if (!existing) throw new ApiError(404, "PRESET_NOT_FOUND", "Không tìm thấy Card Product."); const entries = Object.entries(request.body ?? {}).filter(([key]) => updateFields.has(key)); if (!entries.length) throw new ApiError(400, "INVALID_REQUEST", "Không có field catalog hợp lệ để cập nhật."); const product = normalizeCatalogProduct({ ...existing, ...Object.fromEntries(entries) }); await validateWholeCatalog(catalog, product, existing.presetId); const updated = await catalog.updateProduct(existing.presetId, product); await writeAudit({ event: "CATALOG_PRODUCT_UPDATED", actor: session, request, resource: { type: "catalog_product", id: existing.presetId, providerCode: product.providerCode, fields: entries.map(([key]) => key) } }); return { data: withLegacyAliases(updated!), audit: auditFor(session) }; });
  app.patch<{ Params: { providerCode: string }; Body: { providerName?: unknown; active?: unknown } }>("/api/admin/card-catalog/providers/:providerCode", async (request) => { const session = admin(request); const providerCode = request.params.providerCode.toUpperCase(); const update = { ...(typeof request.body?.providerName === "string" ? { providerName: request.body.providerName.trim() } : {}), ...(typeof request.body?.active === "boolean" ? { active: request.body.active } : {}) }; if (!Object.keys(update).length) throw new ApiError(400, "INVALID_REQUEST", "Không có field provider hợp lệ để cập nhật."); const all = await catalog.listAllProducts(); const matching = all.filter((p) => p.providerCode === providerCode); if (!matching.length) throw new ApiError(404, "PROVIDER_NOT_FOUND", "Không tìm thấy provider.", { provider: providerCode }); const issues = validateCatalogProducts(all.map((p) => p.providerCode === providerCode ? { ...p, ...update } : p)); if (issues.length) throw invalidCatalogError(issues); const affectedProducts = await catalog.updateProvider(providerCode, update); await writeAudit({ event: "CATALOG_PROVIDER_BULK_UPDATED", actor: session, request, resource: { type: "catalog_provider", id: providerCode, providerCode, affectedProducts, fields: Object.keys(update) } }); return { data: { providerCode, affectedProducts }, audit: auditFor(session) }; });
  return app;
};
