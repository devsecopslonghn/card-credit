...
export interface MasterdataRepository {
  list(kind: "banks" | "cardtypes", sortField: string): Promise<MasterRecord[]>;
...
export class MongoMasterdataRepository implements MasterdataRepository {
  ...
  async list(kind: "banks" | "cardtypes", sortField: string) { return this.collection(kind).find().sort({ [sortField]: 1 }).toArray(); }
...
export class InMemoryMasterdataRepository implements MasterdataRepository {
  ...
  async list(kind: "banks" | "cardtypes", sortField: string) { return structuredClone(this.values[kind]).sort((a, b) => String(a[sortField]).localeCompare(String(b[sortField]))); }
