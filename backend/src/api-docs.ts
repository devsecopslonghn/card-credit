...
export const registerApiDocs = async (app: FastifyInstance) => {
  await app.register(swagger, { mode: "static", specification: { path: "", document: ({
    ...
    "x-mcp": { transport: "Streamable HTTP", endpoint: "/mcp", authentication: "Authorization: Bearer <MCP_HTTP_TOKEN>", fixedContext: ["MCP_USER_ID", "MCP_WORKSPACE_ID"], tools: ["get_statement_summary", "list_transactions", "compare_cards", "list_upcoming_statements", "preview_create_transaction", "confirm_create_transaction", "preview_change_statement_payment_status", "confirm_change_statement_payment_status"], mutationPolicy: "Preview -> explicit human confirmation -> idempotent confirm -> audit" },
