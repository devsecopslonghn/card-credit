...
export const registerCalendarSubscriptionRoutes = (app: FastifyInstance, users: AuthRepository, secret: string) => {
  ...
  app.get<{ Params: { token: string } }>("/api/calendar-subscriptions/feed/:token.ics", { logLevel: "silent" }, async (request, reply) => {
    ...
    const inputs = await Promise.all(statements.map(async (statement) => { const item = statement as Data; const card = cardById.get(String(item.userCardId))!; const totals = await CardTransactionModel.aggregate([{ $match: { statementId: item._id } }, { $group: { _id: null, amount: { $sum: "$outcomeAmount" } } }]); return { identity: `${subscription.workspaceId}/${subscription.userId}/${String(item._id)}`, displayName: String(card.displayName ?? card.name ?? "Thẻ tín dụng"), providerName: String(card.providerName ?? card.bank ?? "Ngân hàng"), owner: String(card.owner ?? "Tôi"), periodStartDate: String(item.periodStartDate), periodEndDate: String(item.periodEndDate), statementDate: String(item.statementDate), paymentDueDate: String(item.paymentDueDate), totalAmountDue: Number(totals[0]?.amount ?? 0), effectivePaymentStatus: effectivePaymentStatus(item), timezone: String(card.reminderTimezone ?? "Asia/Ho_Chi_Minh") }; }));
