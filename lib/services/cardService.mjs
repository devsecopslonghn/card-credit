...
export const buildCardSnapshotFromProduct = (product, owner) => {
...
export const createCardFromPreset = async (input, deps = {}) => {
  ...
  return CardModel.create(buildCardSnapshotFromProduct(product, owner));
};
...
export const createLegacyCard = async (input, deps = {}) => {
  ...
  console.warn("Deprecated legacy POST /api/cards contract used.");
  const CardModel = modelFrom(deps);
  return CardModel.create(payload);
};
...
export const buildAllowedUpdate = (input) => {
  ...
  for (const [field, value] of Object.entries(input)) {
    if (OPERATIONAL_UPDATE_FIELDS.has(field)) {
      ...
      if (field === "statementDate") update.statementDate = asOptionalString(value, field) ?? "";
      if (field === "paymentDueDate") update.paymentDueDate = asOptionalString(value, field) ?? "";
      if (field === "amountDueThisMonth") update.amountDueThisMonth = asOptionalNumber(value, field);
      if (field === "isPaidThisMonth") update.isPaidThisMonth = asOptionalBoolean(value, field);
      if (field === "monthlyData") update.monthlyData = validateMonthlyData(value);
      continue;
    }
