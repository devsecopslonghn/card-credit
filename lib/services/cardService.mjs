...
export const buildAllowedUpdate = (input) => {
  ...
  for (const [field, value] of Object.entries(input)) {
    if (OPERATIONAL_UPDATE_FIELDS.has(field)) {
      ...
      if (field === "cashbackCapAmount") {
        const cashbackCapAmount = value === null ? null : asOptionalNumber(value, field);
        if (cashbackCapAmount !== null && cashbackCapAmount !== undefined && cashbackCapAmount < 0) {
          throw apiError(400, "INVALID_REQUEST", "Request body không hợp lệ.", {
            cashbackCapAmount: "cashbackCapAmount phải >= 0 hoặc null.",
          });
        }
        update.cashbackCapAmount = cashbackCapAmount ?? null;
      }
      if (field === "cashbackCapPeriod") {
        const cashbackCapPeriod = asOptionalString(value, field) ?? "STATEMENT";
        if (!["STATEMENT", "CALENDAR_MONTH"].includes(cashbackCapPeriod)) {
          throw apiError(400, "INVALID_REQUEST", "Request body không hợp lệ.", {
            cashbackCapPeriod: "cashbackCapPeriod không hợp lệ.",
          });
        }
        update.cashbackCapPeriod = cashbackCapPeriod;
      }
