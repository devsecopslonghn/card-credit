...
export const buildTransactionReportSummary = ({ cards = [], statements = [], transactions = [], filters = {} }) => {
  ...
  const totals = cardSummaries.reduce(
    (acc, card) => ({
      ...
      cashbackByRate: acc.cashbackByRate + card.totals.cashbackByRate,
      eligibleCashback: acc.eligibleCashback + card.totals.eligibleCashback,
      exceededCashback: acc.exceededCashback + card.totals.exceededCashback,
