...
### Checkpoint: planning/report UI acceptance

- `DECISION-PLAN-01` giữ recurring schedule-only; UI không tự sinh
  `FinancialTransaction`.
- Playwright desktop runtime test `2/2` đã cover report date filter, budget
  upsert và recurring create/deactivate với mocked API boundary; không
  persistence thật. Evidence đã push tại `b8efa4c`.
- SRS `GAP-UI-02/03` chỉ claim CLOSED trong phạm vi này vì có source/contract,
  REST adapter và browser evidence; generation tương lai vẫn phải qua decision
  riêng.

### Checkpoint: authenticated read-only MCP performance profile

- Runtime context đã xác minh tại Kubernetes context `k8s-admin-public`,
  namespace `card-credit`; profile chạy trong backend pod hiện hành và không
  thay đổi deployment/pod/database.
- MCP protocol initialize tạo session thành công; toàn bộ `11/11` query tools
  trong read-only inventory trả HTTP `200`: statement summary, transactions,
  monthly cash-flow, compare cards, duplicate cards, card fees, Fee Center,
  monthly cashbacks, upcoming statements, accounts và personal summary.
- Eight bounded query tools chạy `3` lần: `list_transactions` p50/max
  `11/22ms`, `list_accounts` `9/66ms`, `get_monthly_cash_flow` `10/64ms`,
  `compare_cards` `107/200ms`, `list_duplicate_cards` `5/17ms`,
  `list_fee_center` `5/6ms`, `list_upcoming_statements` `40/43ms`,
  `get_personal_finance_summary` `13/31ms`. Ba query cần parent ID được seed
  bằng read-only `compare_cards`/`list_upcoming_statements`, sau đó đều HTTP
  `200`. Không ghi raw IDs, amounts, token hoặc response payload.
- Independent review: GO cho read-only performance evidence; không gọi
  preview/confirm, không gửi mail, không reconcile/apply dữ liệu.
- Evidence commit: `bdaa09c` đã push lên `origin/master`.

### Checkpoint: security context regression hardening
