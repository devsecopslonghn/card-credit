...
- Argo read-only evidence: Kubernetes `Application/card-credit` CRD reports
  ...
  old GitOps revision is healthy, not that source `51d3a79` has been built or
  rolled out.
- Review conclusion: local source/shared/backend/frontend evidence đã pass,
  nhưng candidate image publication, GitOps handoff cho `51d3a79`, Argo
