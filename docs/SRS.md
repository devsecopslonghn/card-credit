...
| GAP-STM-01 | Đã xử lý một phần | Statement Read v1, notification, private calendar feed, payment-reminder và one-off calendar-email projections đã dùng shared `StatementDto`, `StatementQueryService` và persisted `creditDebt`/impact cho REST GET, MCP summary/upcoming, Frontend read, notifications, ICS và scheduled/one-off email; calendar feed vẫn giữ `lastAccessedAt` write hiện hữu nhưng không đổi schema; payment PATCH và `creditStatements` report projection vẫn còn legacy projection/formula cần slice riêng. |
