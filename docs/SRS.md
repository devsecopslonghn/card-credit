...
| Financial Planning | Category, budget status, recurring expense list/create | Planning write model; status/usage do backend tính | Recurring update/delete/generation chưa có |
...
|---|---|---|---|
| P0 | `GAP-CI-01` | Đã có evidence CI/source publication và candidate read-only từ Jenkins `#368`/tag `8f5c7a0765d5`; proxy cleanup commit `0e6e8e8` có local validation pass và Jenkins `#370` checkout/test pass, nhưng build bị abort thủ công trong bước `skopeo copy`, registry không có tag `0e6e8e883cf1`; runtime vẫn là `3fffccda40e8`, Argo vẫn `Synced/Healthy` revision `9d5fc37` | Giữ regression gate; cần một Jenkins run hoàn tất publication cho `0e6e8e8` trước khi claim image/GitOps/runtime; không suy diễn writer rollout từ Jenkins success |
