...
| GAP-DOC-01 | Đã xử lý một phần | Smoke test đã dùng route canonical `/api/financial-reports/summary` và kiểm tra shape `range/totals`; `docs/api.md`, `docs/requirements.md`, `backend/README.md` và legacy `frontend/lib/api/reportsCore.mjs` vẫn còn mô tả/path cũ cần compatibility-removal slice. |
