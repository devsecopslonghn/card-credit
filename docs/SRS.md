...
| GAP-PAY-01 | Đã xử lý một phần | `/payments` và `/cards` đã chọn account `REAL_MONEY`, gửi `repaymentAccountId` và giữ một `Idempotency-Key` cho mỗi user action; backend không còn fallback environment. REST payment PATCH đã dùng generic receipt/audit guard và canonical safe result. Còn thiếu browser/MCP preview-confirm one-time, reversal và MCP payment command. |
