...
| GAP-STM-01 | Đã xử lý một phần | Statement Read v1 đã đưa shared `StatementDto`, `StatementQueryService`, REST GET, MCP summary/upcoming và Frontend read về persisted `creditDebt`/impact; payment PATCH, one-off calendar email, notification/calendar feed, reminder và `creditStatements` report projection vẫn còn legacy projection/formula cần slice riêng. |
