...
| GAP-STM-01 | Đã xử lý một phần | Statement Read v1, notification projection và private calendar feed đã dùng shared `StatementDto`, `StatementQueryService` và persisted `creditDebt`/impact cho REST GET, MCP summary/upcoming, Frontend read, notifications và ICS; calendar feed còn giữ `lastAccessedAt` write hiện hữu nhưng không đổi schema; payment PATCH, one-off calendar email, reminder và `creditStatements` report projection vẫn còn legacy projection/formula cần slice riêng. |
