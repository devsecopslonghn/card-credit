...
| GAP-DOC-01 | Đã xử lý một phần | Smoke test và Cards JSON export đã dùng route canonical `/api/financial-reports/summary` với shape `range/totals`; legacy `frontend/lib/api/reportsCore.mjs` đã xóa. `docs/api.md`, `docs/requirements.md` và `backend/README.md` vẫn còn mô tả/path cũ cần docs cleanup riêng. |
