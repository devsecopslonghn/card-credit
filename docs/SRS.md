...
|---|---|---|---|
| P0 | `GAP-CI-01` | Đã đóng cho CI/source publication và candidate read-only: Jenkins `#364` checkout commit docs/evidence `eea32cf`, pass 25/45/135, publish immutable images và handoff GitOps `2ac41d8`; runtime mới đang chờ reconcile, candidate trước đó đã có health/ready/MCP read smoke | Giữ regression gate; không suy diễn writer rollout từ Jenkins success |
