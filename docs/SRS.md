...
| P1 | `GAP-REP-01` | **CLOSED** — `DECISION-REP-01` chốt ledger `categoryId` là source authoritative, category catalogue chỉ là planning metadata; report regression chứng minh không đọc category catalogue/legacy `monthlyData`, category REST canonical được trusted-context test và endpoint default legacy đã xóa; live report read/reconciliation đã pass | Giữ ledger/category no-join guard và stale endpoint check trước release |
