...
| P0 | `GAP-MCP-01` | **PARTIAL** — canonical manifest, preview/confirm/idempotency guard, REST/MCP adapter tests và chart desired-state read-only guard đã có evidence; live pod hiện `MCP_WRITER_MODE=read` với health/readiness/docs `200`, nhưng chưa claim external old-writer traffic fence hoặc financial receipt/audit/reconciliation từ traffic/mutation thật | Xác minh external old-writer traffic fence, giữ preview/confirm/idempotency/audit và production receipt/reconciliation evidence |
...
| P1 | `GAP-DATA-01`, `GAP-ACC-01`, `GAP-DATA-02` | **IN_PROGRESS** — policy card soft-retire/restricted-merge, account retention, calendar partial-unique index và deterministic duplicate-card preflight source/test đã có; live dry-run/index rollout và reconciliation evidence còn thiếu | Additive index dry-run/apply, lifecycle tests và reconciliation evidence |
...
| P1 | `GAP-REP-02` | **PARTIAL** — authoritative source là financial ledger + statement projection; `cardId` và `owner/year/month` filters đã có REST/MCP/UI parity theo statement/account reference; deterministic orphan inventory đã có source/test/operator audit nhưng chưa có live workspace result và completeness evidence | Chạy audit read-only trên snapshot được phép, thêm completeness evidence |
...
| P1 | `GAP-AUTH-01` | **PARTIAL** — forgot-password đã nối MailService với generic response và test delivery; runtime metadata có `SMTP_HOST`/`SMTP_USER` nhưng chưa có SMTP connectivity/delivery hoặc sender-owner evidence | Chạy connectivity/delivery test với recipient được phép và ghi owner evidence |
