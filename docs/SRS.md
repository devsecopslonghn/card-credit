...
| GAP-STM-01 | Đã xử lý một phần | Statement Read v1, notification, private calendar feed và payment-reminder projection đã dùng shared `StatementDto`, `StatementQueryService` và persisted `creditDebt`/impact cho REST GET, MCP summary/upcoming, Frontend read, notifications, ICS và scheduled email; calendar feed vẫn giữ `lastAccessedAt` write hiện hữu nhưng không đổi schema; payment PATCH, one-off calendar email và `creditStatements` report projection vẫn còn legacy projection/formula cần slice riêng. |
