...
| P0 | `GAP-SEC-01`, `GAP-SEC-02` | **PARTIAL** — source guard, authoritative lookup, workspace/session checks, authoritative role refresh, inactive/locked MCP rejection, atomic `$inc sessionVersion` và MCP provider bind/reject theo `sessionVersion` đã có regression evidence; authoritative policy-membership runtime evidence còn thiếu | Backend/frontend tests, runtime authoritative policy membership và deployed-image evidence |
