...
| P0 | `GAP-MCP-01` | **PARTIAL** — canonical manifest, preview/confirm/idempotency guard, REST/MCP adapter tests và chart desired-state read-only guard đã có evidence; live pod vẫn `MCP_WRITER_MODE=write` nên chưa claim external old-writer fence hoặc financial receipt/audit/reconciliation từ traffic/mutation thật | Đồng bộ candidate read-only khi được phép, xác minh external old-writer traffic fence, giữ preview/confirm/idempotency/audit và production receipt/reconciliation evidence |
