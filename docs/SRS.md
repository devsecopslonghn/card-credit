...
|---|---|---|---|
| P0 | `GAP-CI-01` | Proxy cleanup commit `0e6e8e8` đã được Jenkins `#373` checkout qua source commit `19f068f`, pass shared/frontend/backend `25/45/135`, build output nhận diện `ƒ Proxy (Middleware)`, publish immutable images tag `19f068f18d53` và handoff GitOps chart `952c0fc`; runtime vẫn là `3fffccda40e8`, Argo vẫn `Synced/Healthy` revision `9d5fc37` | Xác minh Argo reconcile và runtime health/ready/docs read-only cho tag mới; không suy diễn writer rollout từ Jenkins success |
