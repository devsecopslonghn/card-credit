...
| Financial Ledger | Account, transaction, persisted financial impact, reports | Một service/query/command cho REST, MCP, UI; integer VND | Current source có zero `McpMutationModel` create/update; vẫn đọc legacy receipt để chống replay trùng, nên chỉ xóa sau fence/drain và reconciliation |
...
- **DECISION-REP-01 — Category source**: `FinancialTransaction.categoryId` là
  giá trị authoritative đã persisted để nhóm report và budget usage. `FinanceCategory`
  chỉ là planning catalogue tùy chọn để hiển thị/kiểm soát nhãn; report không
  join catalogue, không rebuild ledger từ category metadata và không tự backfill
...
| P0 | `GAP-SEC-01`, `GAP-SEC-02` | **CLOSED** — source guard, authoritative lookup, workspace/session checks, authoritative role refresh giữa MCP invocations, inactive/locked MCP rejection, atomic `$inc sessionVersion`, MCP provider bind/reject theo `sessionVersion`; UAT runtime xác nhận configured MCP user tồn tại, active, unlocked, role `admin`, workspace match, sessionVersion hợp lệ và authenticated `list_accounts` trả `200` | Giữ authoritative lookup và runtime guard trong release gate |
| P0 | `GAP-MCP-01` | **CLOSED** — canonical manifest, preview/confirm/idempotency guard, user-approved `DECISION-MCP-WRITE-01`, live `MCP_WRITER_MODE=write`/`MCP_OLD_WRITER_FENCED=true`, health/readiness/docs `200`, unauthenticated `/mcp` `401`, 17-tool inventory; full-cluster chỉ có canonical backend/frontend workload, ingress UAT 24h window ghi nhận card-host `/mcp=0` và `5xx=0`; authenticated UAT `preview_create_account` → `confirm_create_account` → replay đều `200`, một `COMPLETED` receipt và một audit; source audit không có `McpMutationModel` create/update | Giữ writer fence, no-mixed-writer inventory và receipt/audit monitoring trong release gate |
| P0 | `GAP-PAY-01`, `GAP-PAY-02`, `GAP-STM-01` | **CLOSED** — payment contract, state-machine guard, preview/CAS, command tests và `DECISION-PAY-REV-01`; fresh UAT backup trước payment; canonical preview/confirm/replay đều `200`, statement chuyển `PAID`, đúng một `STATEMENT_PAYMENT`, một `COMPLETED` receipt và một audit; post-check `8/8` payment sync, `missingPayments=[]`, orphan records `0`, duplicate guards `0` | Giữ preview/CAS/idempotency/audit, PAID lock và no-reversal guard trong release gate |
...
| P1 | `GAP-REP-02` | **CLOSED** — authoritative source là financial ledger + statement projection; `cardId` và `owner/year/month` filters đã có REST/MCP/UI parity theo statement/account reference; UAT post-payment audit cho workspace MCP cố định ghi nhận 5 cards, 11 statements, 46 financial transactions, `8/8` paid-statement payment sync, `missingPayments=[]` và orphan records `0` | Giữ read-only orphan/completeness audit trước release |
...
| P1 | `GAP-AUTH-01` | **CLOSED** — forgot-password nối MailService với generic response; test delivery và regression SMTP failure không lộ reset link; UAT delivery tới configured MCP user trả `200`, response generic không có reset link, audit `PASSWORD_RESET_REQUESTED` ghi `delivered=true`, SMTP config đầy đủ | Giữ generic response, no-token logging và SMTP failure audit |
...
| P2 | `GAP-PERF-01` | **CLOSED** — bounded transaction, calendar subscription, workspace notes, recurring, legacy masterdata, card, duplicate-card, category, fee, admin-audit và query-form admin-user reads; report aggregation consume complete sources qua bounded Mongo cursor; card-statement/credit-statement projections có stable opaque cursor/page contract và REST/frontend regression evidence; live authenticated MCP profile initialize thành công, toàn bộ `11/11` query tools trả HTTP `200`, gồm parent-ID seeded reads, và không có mutation tool được gọi | Giữ bounded/paginated coverage và read-only profile trước release |
