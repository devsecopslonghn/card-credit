...
| P0 | `GAP-MCP-01` | **PARTIAL** — canonical manifest, preview/confirm/idempotency guard, REST/MCP adapter tests và chart desired-state read-only guard đã có evidence; source audit xác nhận `McpMutationModel` chỉ còn `findOne` compatibility reads, không có create/update; live authenticated `tools/list` trả 11 query tools, không có preview/confirm, health/readiness/docs đều `200`; vẫn chưa claim external old-writer traffic fence hoặc financial receipt/audit/reconciliation từ traffic/mutation thật | Xác minh external old-writer traffic fence, giữ preview/confirm/idempotency/audit và production receipt/reconciliation evidence; chỉ xóa legacy receipt read sau migration/reconciliation evidence |
