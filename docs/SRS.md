...
| Statement | `POST /api/cards/:id/statements/:statementId/payment/preview` | Session | Read-only persisted-impact preview, không ghi database |
| Statement | `PATCH /api/cards/:id/statements/:statementId/payment` | Session + `Idempotency-Key` | State/payment mutation qua generic receipt/audit guard |
...
| GAP-PAY-01 | Đã xử lý một phần | `/payments` và `/cards` đã chọn account `REAL_MONEY`, gọi browser read-only preview để hiển thị exact `amountToPay`/account requirement, gửi `repaymentAccountId` và giữ một `Idempotency-Key` cho mỗi user action; backend không còn fallback environment. REST payment PATCH đã dùng generic receipt/audit guard và canonical safe result. Còn thiếu one-time/resource-version confirm, reversal và MCP payment command. |
| GAP-PAY-02 | Đã xử lý một phần | Payment command dùng strict action, card/workspace scope, persisted `creditDebt`, Mongo transaction, PAID lock, generic receipt/audit cùng transaction và unique partial index cho một `STATEMENT_PAYMENT`; `PAID` lặp lại là idempotent, duplicate unique payment chỉ retry bounded một lần. `REOPEN` sau `PAID` vẫn bị khóa và chưa có reversal/compensating transaction; resource-version/one-time confirmation/MCP follow-up vẫn mở. |
| GAP-STM-01 | Đã xử lý một phần | Statement Read v1, notification, private calendar feed, payment-reminder, one-off calendar-email và `creditStatements` report projections dùng shared `StatementDto`, `StatementQueryService` và persisted `creditDebt`/impact cho REST GET, MCP summary/upcoming, Frontend read, notifications, ICS, scheduled/one-off email và credit report. Payment preview hiện dùng shared `StatementPaymentPreviewDto` read-only; PATCH đi qua `StatementPaymentCommandService`, strict shared input và generic guard rồi trả canonical DTO. Calendar feed vẫn giữ `lastAccessedAt` write hiện hữu; one-time/resource-version confirm, reversal và MCP mutation còn là follow-up. |
