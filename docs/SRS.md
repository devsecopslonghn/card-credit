...
| P0 | `GAP-MCP-01`, `GAP-PAY-01`, `GAP-PAY-02`, `GAP-STM-01` | User đã quyết định cho canonical MCP writer chuyển sang `write` và xác nhận không cần legacy old writer; chart commit `f4eb8b5` đặt `writerMode: write`, `oldWriterFenced: true` trên image `e1ce0fe53242`; chưa gọi mutation và runtime write-mode chưa được claim | Verify Argo/runtime MCP manifest ở `write` read-only metadata trước mọi financial command; giữ preview/confirm/idempotency/audit, resource/HITL policy và reversal decision |
