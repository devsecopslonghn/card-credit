...
| ACC-03 | Chỉ account `CREDIT` được nhận `creditCardId`; một card tối đa liên kết một account nhờ unique partial index. |
...
| MCP-03 | Read tools hiện có: `get_statement_summary`, `list_transactions`, `compare_cards`, `list_upcoming_statements`, `get_personal_finance_summary`, `list_accounts`. |
...
| GAP-STM-01 | Cao | Legacy statement adapter dùng `serviceFeeRate` như cashback rate và cộng `amount` của mọi non-payment transaction vào total due. Semantics này không đồng nhất với Financial Domain và có thể làm sai due/cashback khi có refund/reimbursement/cashback. |
...
| GAP-MCP-01 | Cao | Confirmation token MCP hiện có thể được dùng lại tới khi hết hạn, chỉ bind operation/payload/expiry, chưa bind actor/workspace/resource version hoặc one-time human confirmation. `McpMutationModel` là idempotency receipt, chưa phải append-only audit; dùng cùng preview với idempotency key khác có thể tạo effect lặp. |
...
| GAP-WEB-01 | Trung bình | Next middleware chưa guard các UI `/dashboard`, `/transactions`, `/accounts`, `/budgets`, `/reports`, `/payments`, `/notifications`, `/fees`, `/cashback`; backend API vẫn chặn nên data không lộ, nhưng UX redirect không nhất quán. |
| GAP-API-01 | Trung bình | Static OpenAPI thiếu nhiều endpoint admin/cashback/fee/workspace và `x-mcp` còn liệt kê bốn tool đã loại bỏ. Swagger chưa là contract đầy đủ của runtime. |
| GAP-DOC-01 | Trung bình | `docs/api.md`, `docs/requirements.md`, `backend/README.md` và smoke test còn nhắc `/api/reports/summary`/startup import behavior không khớp route hiện tại `/api/financial-reports/summary`. |
| GAP-PERF-01 | Thấp-Trung bình | Hầu hết list API chưa pagination. `upcomingStatements()` tính summary tuần tự từng statement; MCP transport map phụ thuộc client close để giải phóng. |
