...
| P0 | `GAP-SEC-01`, `GAP-SEC-02` | **PARTIAL** — source guard, authoritative lookup, workspace/session checks, authoritative role refresh giữa MCP invocations, inactive/locked MCP rejection, atomic `$inc sessionVersion` và MCP provider bind/reject theo `sessionVersion` đã có regression evidence; authoritative policy-membership runtime evidence còn thiếu | Backend/frontend tests, runtime authoritative policy membership và deployed-image evidence |
| P0 | `GAP-MCP-01` | **PARTIAL** — canonical manifest, preview/confirm/idempotency guard, REST/MCP adapter tests và chart desired-state read-only guard đã có evidence; source audit và regression `legacy-writer-fence.test.ts` xác nhận `McpMutationModel` chỉ còn `findOne` compatibility reads, không có create/update; live authenticated `tools/list` trả 11 query tools, không có preview/confirm, health/readiness/docs đều `200`; vẫn chưa claim external old-writer traffic fence hoặc financial receipt/audit/reconciliation từ traffic/mutation thật | Xác minh external old-writer traffic fence, giữ preview/confirm/idempotency/audit và production receipt/reconciliation evidence; chỉ xóa legacy receipt read sau migration/reconciliation evidence |
...
| P1 | `GAP-AUTH-01` | **PARTIAL** — forgot-password đã nối MailService với generic response; test delivery và regression SMTP failure chứng minh không lộ reset link, audit `delivered=false`; runtime SMTP `transport.verify()` pass tới port `587`/`secure=false`, nhưng chưa có approved-recipient delivery hoặc sender-owner evidence | Gửi connectivity-safe delivery test tới recipient được phép và ghi sender-owner evidence |
