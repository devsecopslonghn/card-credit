...
| CAT-07 | Runtime không được tự động import hoặc overwrite catalog baseline khi start; catalog baseline chỉ được import qua CLI dry-run/apply có operator review và production guard. |
...
| STM-11 | Legacy payment reconciliation mặc định là dry-run/quarantine; chỉ được mark statement `PAID` sau operator approval với exact statement/transaction target, backup private, reviewed/current source-plan hash, trusted admin job, idempotency và conditional `updatedAt` precondition. Không xóa payment transaction để sửa state. |
...
| GAP-PAY-01 | Đã xử lý một phần | `/payments` và `/cards` chọn account `REAL_MONEY`, gọi browser preview canonical để hiển thị exact `amountToPay`/account requirement, giữ `previewId` + token + `expectedVersion` và một `Idempotency-Key` cho mỗi user action; backend không fallback environment. PATCH verify HMAC domain riêng cho browser, bind workspace/user/channel, card/statement/action/account/version, one-time consume qua generic guard và trả canonical safe result. Legacy payment planner/quarantine và guarded operator mark-paid đã xử lý 2 full-settlement cases; MCP preview/confirm đã dùng cùng canonical payload/service. Vẫn thiếu reversal. |
| GAP-PAY-02 | Đã xử lý một phần | Payment command dùng strict action, card/workspace scope, persisted `creditDebt`, Mongo transaction, preview-version CAS trên `updatedAt`, PAID lock, generic receipt/audit cùng transaction và unique partial index cho một `STATEMENT_PAYMENT`; `PAID` lặp lại là idempotent, duplicate unique payment chỉ retry bounded một lần. Legacy drift dùng quarantine rồi mark-paid operator command với source/plan hash, backup và CAS; không sửa ledger transaction. MCP payment preview/confirm đã delegate cùng command service/guard và không direct model. `REOPEN` sau `PAID` vẫn bị khóa và chưa có reversal/compensating transaction; old-writer rollout gate còn mở. |
| GAP-STM-01 | Đã xử lý một phần | Statement Read v1, notification, private calendar feed, payment-reminder, one-off calendar-email và `creditStatements` report projections dùng shared `StatementDto`, `StatementQueryService` và persisted `creditDebt`/impact cho REST GET, MCP summary/upcoming, Frontend read, notifications, ICS, scheduled/one-off email và credit report. Payment preview/execute dùng shared strict contracts; preview ghi hash-only confirmation metadata, PATCH yêu cầu browser trusted confirmation, one-time guard, preview-version CAS và trả canonical DTO. Legacy payment reconciliation đã dry-run/quarantine, audit và mark-paid 2 cases sau operator review; Calendar feed vẫn giữ `lastAccessedAt` write hiện hữu; reversal và MCP mutation còn là follow-up. |
...
| GAP-OPS-01 | Đã xử lý | `server.ts` không còn gọi `syncCatalogFromFile()` khi start. Runtime chỉ kết nối database, phục vụ API và readiness; catalog baseline được import riêng qua CLI dry-run/apply với production guard, nên restart không overwrite admin catalog changes. |
| GAP-MCP-01 | Đã xử lý một phần | Preview token v2 dùng dedicated `MCP_PREVIEW_SECRET`, TTL 300 giây, HMAC domain separation, canonical payload hash, `previewId` và bind workspace/user/channel; token không chứa raw payload, persistent `commandpreviews` chỉ lưu token/payload hash. Generic `CommandReceipt`/`CommandAudit` đã wire vào Account, Financial Transaction và Statement Payment command service cho REST (`Idempotency-Key`) và MCP confirm; `CommandGuardService` consume one-time atomically, replay completed receipt trước expiry và rollback consume khi business fail. MCP payment preview/confirm dùng chung canonical payload với REST. Còn thiếu resource-version binding đầy đủ, trusted host/HITL receipt, failure-audit policy và production index/old-writer rollout gate. |
