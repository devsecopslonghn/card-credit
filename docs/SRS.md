...
| GAP-REP-01 | Đã xử lý một phần | `FinancialReportService` và shared `FinancialReportDto` đã đọc ledger transaction, `MonthlyCardCashbackModel` và `CardFeePaymentModel` theo workspace/range; report summary hiện dùng shared strict date-range contract với REST/MCP/Frontend parity (REST default current-month → today, MCP yêu cầu đủ range, ngày sai/range đảo/filter dư fail-closed). REST, MCP `get_personal_finance_summary`, frontend client/report page dùng cùng DTO. Card fee history/Fee Center GET, monthly cashback GET và monthly cash-flow GET dùng shared read schemas + query services; fee/cashback REST mutations đã được đưa vào `FeeCommandService`/`MonthlyCashbackCommandService` với trusted context, nhưng vẫn là compatibility commands chưa có generic preview-confirm/idempotency/audit và chưa có MCP mutation. Cash-flow vẫn giữ extraction formula và chưa follow `reimbursementForTransactionId` về expense CREDIT. `credit-statements` vẫn giữ validation AS-IS; owner/card/year/month filters, orphan cleanup, semantic cash-flow repair và legacy fee-category migration cần slice riêng; vì vậy chưa đóng hoàn toàn FR-08. |
