...
| P0 | `GAP-MCP-01` | **PARTIAL** — canonical manifest, preview/confirm/idempotency guard, user-approved `DECISION-MCP-WRITE-01`, live `MCP_WRITER_MODE=write`/`MCP_OLD_WRITER_FENCED=true`, health/readiness/docs `200`, unauthenticated `/mcp` `401`, 17-tool inventory và full-cluster absence of an application old-writer workload; authenticated UAT `preview_create_account` → `confirm_create_account` → replay đều `200`, `accountCount=1`, `receiptCount=1`, `auditCount=1`, receipt `COMPLETED`; source audit không có `McpMutationModel` create/update, nhưng external drain chưa chứng minh | Evidence external old-writer fence/drain và continued authenticated receipt/audit monitoring; remove legacy receipt read only after those gates |
| P0 | `GAP-PAY-01`, `GAP-PAY-02`, `GAP-STM-01` | **PARTIAL** — payment contract, state-machine guard, preview/CAS, command tests và `DECISION-PAY-REV-01` fail-closed boundary; UAT backup trước migration, reconciliation dry-run 4 workspace với `missingPayments=0`, candidate/quarantine `0`, additive command/data-integrity indexes apply + verify đã có; chưa chạy authenticated business mutation smoke | Authenticated command persistence/audit smoke; không tự mark-paid hoặc reversal |
