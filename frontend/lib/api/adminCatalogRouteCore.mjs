...
import { canManageCatalog } from "../auth/rbacCore.mjs";
...
const auditCatalogEvent = async ({ AuditLogModel, connectToDatabase, ...event }) => {
  if (AuditLogModel && connectToDatabase) await connectToDatabase();
  await logAuthEvent({ AuditLogModel, ...event });
};

export const createAdminCatalogRouteHandlers = ({ readCatalogProducts, writeCatalogProducts, requireAuth, AuditLogModel, connectToDatabase }) => ({
  ...
  async createProduct(request) {
    try {
      ...
      await auditCatalogEvent({
        AuditLogModel,
        connectToDatabase,
        event: "CATALOG_PRODUCT_CREATED",
        request,
        actor: session,
        resource: {
    ...
    try {
      ...
      await auditCatalogEvent({
        ...
        resource: {
          type: "catalog_product",
          id: presetId,
          providerCode: nextProducts[index].providerCode,
  ...
  async updateProvider(request, context) {
    try {
      ...
      await auditCatalogEvent({
        AuditLogModel,
        connectToDatabase,
        event: "CATALOG_PROVIDER_BULK_UPDATED",
        request,
        actor: session,
        resource: {
          type: "catalog_provider",
          id: normalizedProviderCode,
          providerCode: normalizedProviderCode,
          affectedProducts: changed,
          fields: ["providerName", "active"].filter((field) => field in body),
        },
      });
