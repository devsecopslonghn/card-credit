...
const getCardCashbackConfig = (card) => ({
  cashbackCapAmount: card?.cashbackCapAmount ?? null,
  cashbackCapPeriod: card?.cashbackCapPeriod ?? CASHBACK_CAP_PERIODS.STATEMENT,
});
...
const serializeStatement = (statement, transactions = [], card = null) => {
  const plain = statement?.toObject?.() ?? { ...statement };
  const summary = summarizeTransactions(transactions, {
    ...getCardCashbackConfig(card),
    statement: plain,
  });
  return {
    ...plain,
...
export const serializeTransaction = (transaction, statement = null, card = null) => {
  ...
  return {
    ...
    derived,
    statement: statement ? serializeStatement(statement, [], card) : undefined,
    card: card
      ? {
          _id: toId(card._id),
...
export const listStatementsForCard = async (cardId, deps = {}, session = null) => {
  ...
  const card = await CardModel.findById(cardId);
  return statements.map((statement) => serializeStatement(statement, transactionsByStatement.get(toId(statement._id)) ?? [], card));
};

export const getStatementDetail = async (cardId, statementId, deps = {}, session = null) => {
  const { CardModel, CardStatementModel, TransactionModel } = deps;
  const card = await assertOwnedCard(cardId, { CardModel, session });
  assertObjectId(statementId, "statementId");
  ...
  return {
    ...serializeStatement(statement, transactions, card),
    transactions: transactions.map((transaction) => serializeTransaction(transaction, statement, card)),
  };
};
...
export const markStatementPaid = async (cardId, statementId, deps = {}, session = null) => {
  ...
  const card = await deps.CardModel.findById(cardId);
  ...
  return serializeStatement(updated, transactions, card);
};
